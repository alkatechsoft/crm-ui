...
console.log('current time',currentDate.getTime("HH:mm"));
   ...
   this.axios.post('http://localhost:8080/lcrm-api/count-client-by-category').then((response)=>{
         if(response.data.response_code === 200){
          ...
          response.data.response_body.map((data) => this.items.push({subscriberType: data.subscriberType + " - " + data.count , count:data.count, id:data.id, userArray:data.userArray, actions:''}));
